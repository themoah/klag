...
public record HotPartitionConfig(
  ...
  public static HotPartitionConfig fromEnvironment() {
    boolean enabled = Env.getBool("HOT_PARTITION_ENABLED", DEFAULT_ENABLED);
    double sigma = Env.getDouble("HOT_PARTITION_SIGMA_MULTIPLIER", DEFAULT_SIGMA_MULTIPLIER);
    int minPartitions = Env.getInt("HOT_PARTITION_MIN_PARTITIONS", DEFAULT_MIN_PARTITIONS);
