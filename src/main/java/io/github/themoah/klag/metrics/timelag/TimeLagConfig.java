...
public record TimeLagConfig(
  ...
  public static TimeLagConfig fromEnvironment() {
    boolean enabled = EnvConfig.getBoolean("TIME_LAG_ENABLED", DEFAULT_ENABLED);
    long minLagMessages = EnvConfig.getLong("TIME_LAG_MIN_MESSAGES", DEFAULT_MIN_LAG_MESSAGES);
    int interpolationBufferSize = EnvConfig.getInt("TIME_LAG_INTERPOLATION_BUFFER_SIZE", DEFAULT_INTERPOLATION_BUFFER_SIZE);
    long staleProducerThresholdMs = EnvConfig.getLong("TIME_LAG_STALE_PRODUCER_THRESHOLD_MS", DEFAULT_STALE_PRODUCER_THRESHOLD_MS);
