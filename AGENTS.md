...
Instructions for AI coding agents and the complete technical reference for this repository.
Read this file before changing code. It is the single source of truth for architecture,
configuration, metrics, and development workflow.
...
Java 21 is required (`sdk use java 21.0.9-tem`). Use `./gradlew` locally; CI calls
`gradle` directly because no wrapper JAR is committed.
...
./scripts/e2e-test.sh          # k3d + real Kafka + chart + lag assertions
./scripts/e2e-strimzi-test.sh  # Strimzi-managed Kafka (KRaft)
./scripts/e2e-strimzi-matrix.sh # Strimzi e2e across supported Kafka versions

cd website && npm run build && npm test
```

The e2e scripts start real clusters, take longer, and require Docker. Do not run them
speculatively.

### GraalVM Native Image

Native builds require a GraalVM JDK 21 (LTS) with `native-image` (for example,
`sdk install java 21.0.2-graalce`). Set `JAVA_HOME`/`GRAALVM_HOME` to that JDK.
...
4. **Adding a metric means updating `dashboard/demo-dashboard.json`** and the metrics tables
   in this file and `website/src/content/docs/metrics/`.
...
Async operations return `Future<T>`. Use Java 21 records for DTOs and SLF4J + Logback
for logging. Config resolves classpath `application.properties` → external
`KLAG_CONFIG_FILE` → `KAFKA_*` env vars; every `Env`-backed variable also resolves
from `-DNAME` and `-Dname.dotted`. Match surrounding comment density and naming rather
than introducing a new style. For application versioning, follow invariant 5 above.
